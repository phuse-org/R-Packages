# # HEADER
#'  Display:     PhUSE Figure 7.1 Box plot - Measurements by Treatments and Visits
...
#  Turn on or off horizontal lines 
#  Select TIFF, JPEG, or PNG and choose file size
#  rename arms (if they are too long)
...
#' @return PhUSE Figure 7.1 Box plot by Treatment and Time
#' @import ggplot2
#' @import data.table
#' @import gridExtra
#' @import Hmisc
#' @import tools
#' @export
BDS_Box_Obs <- function(data, treatmentname, useshortnames = c(TRUE,FALSE),oldnames, newnames,
  ...
  #select population flag to subset on such as SAFFL or ITTFL
  colnames(testresultsread)[names(testresultsread) == popflag] <- "FLAG" 
  ...
  for(i in 1:visitsplits) {
    ...
    if (usepopflag == TRUE){
      ...
      testresults <- subset(testresultsread, PARAMCD == testname & AVISITN %in% 
                            selectedvisits[(initial):(perpage*i)])
    }
    ...
    summary <- buildtable(avalue = quote(CHG), dfname= quote(testresults), 
                          by1 = "AVISITN", by2 = "TREATMENT", dignum)[order(AVISITN, TREATMENT)]
